...
Illustrative images and wiring diagrams are available in the `images/` folder. The analog reading of the signal is performed on pin 36 of the ESP32 (identified as `SP` in the diagrams).
...
- Insert the offset value obtained in the previous step into the `voltageOffset` variable.
- Adjust the `resistance` variable to reflect the actual burden resistor value (measure it with a multimeter).
...
- Open the script in your preferred text editor and replace the `data` list with a list of pairs (measured_value, expected_value).
...
- Insert these coefficients into the corresponding fields in `calibration.ino`:
    double scaleCoefficient = ...;     // Regression slope, the obtained `a`.
    double interceptCoefficient = ...; // Regression intercept, the obtained `b`.
...
Due to its simplicity and low cost, this project has some limitations:
...
However, in industrial environments (with reactive loads), this approach can introduce significant errors. In such cases, it is recommended to add a real-time voltage measurement module to the circuit.
